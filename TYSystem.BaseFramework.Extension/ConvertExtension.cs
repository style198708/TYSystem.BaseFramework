            ...
            if (typeof(TResult).IsClass)
            {
                if (typeof(TResult).IsGenericType)
                {
                    ...
                    foreach (ParameterInfo p in parameters)
                    {
                        ...
                        {
                            itemValue = Convert.ChangeType(propertyInfo.GetValue(value, null), p.ParameterType.GetUnderlyingType());
                        }
                ...
                else
                {
                    ...
                    while (num < properties.Length)
                    {
                        ...
                        {
                            propertyValue = Convert.ChangeType(resultPropertyInfo.GetValue(value, null), propertyInfo.PropertyType.GetUnderlyingType());
                ...
                if (typeof(TResult).IsGenericType)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        foreach (ParameterInfo p in parameters)
                        {
                            ...
                            {
                                itemValue = Convert.ChangeType(propertyInfo.GetValue(item, null), p.ParameterType.GetUnderlyingType());
                            }
                            values[index++] = itemValue;
                        }
                        TResult entity = (TResult)constructor.Invoke(values);
                ...
                else
                {
                    foreach (var item in list)
                    {
                        ...
                        while (num < properties.Length)
                        {
                            ...
                            if (resultPropertyInfo != null)
                            {
                                propertyValue = Convert.ChangeType(resultPropertyInfo.GetValue(item, null), propertyInfo.PropertyType.GetUnderlyingType());
                                propertyInfo.SetValue(dataEntity, propertyValue, null);
                            }
                            num++;
                        }
